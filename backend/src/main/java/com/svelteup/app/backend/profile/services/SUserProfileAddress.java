...
public class SUserProfileAddress extends  SHttpExceptionThrower  implements HttpUsernameService<AddressDto, SvelteUpUserProfileDto> {
    ...
    @Override
    public ResponseEntity<AddressDto> get(String username) throws Http400Exception, Http401Exception, Http403Exception, Http405Exception, NotSupportedException
    {
        SvelteUpUserProfile discoveredAddress = this.svelteUpUserProfileService.findBySurrogateIdWithCheck(username,username);
        AddressDto putAddressDto = discoveredAddress.toAddressDto();
        return ResponseEntity.ok(putAddressDto);
    }

    @Override
    public void put(String authenticatedUser, AddressDto update_DTO) throws Http400Exception, Http401Exception, Http403Exception, Http405Exception, NotSupportedException {
