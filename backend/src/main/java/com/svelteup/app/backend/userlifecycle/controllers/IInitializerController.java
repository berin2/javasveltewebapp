...
public interface IInitializerController {
    ...
    @ResponseStatus(HttpStatus.OK)
    ResponseEntity<AppInitDto> postUserIdentity(@AuthenticationPrincipal SvelteUpUser user, @RequestBody SvelteUpUserAccountDto dto);
    @PostMapping(ApplicationApi.POST_PHONE_NUMBER_VERIFY)
    @ResponseStatus(HttpStatus.OK)
    void postPhoneNumberVerify(@AuthenticationPrincipal SvelteUpUser user, @RequestBody SvelteUpUserAccountDto dto);
