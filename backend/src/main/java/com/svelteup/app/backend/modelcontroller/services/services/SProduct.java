 ...
 * */
@Service
public class SProduct  extends SSurrogateEntity<UUID,Product>
        implements HttpUuidService<PutProductDto, PostProductDto>
{
    ...
    public ResponseEntity<List<PutProductDto>> findAllByUsername(String owning_username) throws NotSupportedException, IOException {
        ...
        if(productEntityList.size() > 0) {
            ...
            for (Product product : productEntityList) {
                imageIterator = this.s3ImageService.getMultipleImages(Product.class,owning_username,Product.MAXIMUM_IMAGES,product.getSurrogateId());
